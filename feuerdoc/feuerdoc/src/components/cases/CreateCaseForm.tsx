...
const CreateCaseForm: React.FC<CreateCaseFormProps> = ({ onCaseCreated, onClose }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 p-1 bg-gray-950 text-gray-100 rounded-lg">
      {error && (
      ...
      <div>
        ...
        <input
          ...
          className="w-full p-3 bg-gray-900 border border-gray-700 rounded-md placeholder-gray-500 focus:ring-2 focus:ring-fire-primary focus:border-fire-primary transition-colors duration-200 text-gray-200"
      ...
      <div className="flex justify-end items-center space-x-4 pt-2">
        <button
          ...
          onClick={onClose}
          disabled={isLoading}
          className="px-5 py-2.5 text-sm font-medium text-gray-400 bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Cancel
        </button>
        <button
